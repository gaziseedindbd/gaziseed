...
import { Plus, X, Copy, Check, Mail, ShieldCheck, Shield, Loader2, RefreshCw, Trash2, Power, AlertTriangle, UserCog, Key, Lock, History } from 'lucide-react';
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative z-10 max-w-md rounded-2xl bg-background p-6">
            ...
            <p className="mb-2 text-sm text-muted-foreground">
              <strong>{tempPasswordEmail}</strong> এর জন্য টেম্পোরারি পাসওয়ার্ড:
            </p>
      ...
      <div className="relative z-10 w-full max-w-md rounded-2xl bg-background p-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className={`rounded-lg border p-3 text-sm ${isMaster ? 'border-primary/30 bg-primary/5' : 'border-border bg-secondary/20'}`}>
            <p className={`font-medium ${isMaster ? 'text-primary' : 'text-foreground'}`}>
              রোল: {isMaster ? 'MASTER_ADMIN' : 'ADMIN'}
            </p>
            <p className="mt-1 text-muted-foreground">
              সিস্টেম একটি টেম্পোরারি পাসওয়ার্ড তৈরি করবে। নতুন {isMaster ? 'মাস্টার অ্যাডমিন' : 'অ্যাডমিন'} প্রথম লগইনে নতুন পাসওয়ার্ড সেট করতে বাধ্য থাকবেন।
            </p>
          </div>
...
function EditAdminModal({ admin, onClose, onSave, loading }: { admin: any; onClose: () => void; onSave: (data: any) => Promise<void>; loading: boolean }) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (phoneTouched && phone.trim()) data.phone = phone.trim();
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative z-10 w-full max-w-md rounded-2xl bg-background p-6">
        <div className="mb-4 flex items-center justify-between">
          <h3 className="flex items-center gap-2 text-lg font-bold"><UserCog className="h-5 w-5" /> এডমিন এডিট করুন</h3>
          <button onClick={onClose}><X className="h-5 w-5" /></button>
        </div>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
          ...
          <div>
            <label className="mb-1 block text-sm font-medium">নাম</label>
            <input type="text" value={name} onChange={(e) => { setName(e.target.value); setNameTouched(true); }} className="input-bangla" placeholder="নতুন নাম লিখুন (খালি রাখলে আগের থাকবে)" />
          </div>
          <div>
            <label className="mb-1 block text-sm font-medium">ফোন</label>
            <input type="tel" value={phone} onChange={(e) => { setPhone(e.target.value); setPhoneTouched(true); }} className="input-bangla" placeholder="নতুন ফোন লিখুন (খালি রাখলে আগের থাকবে)" />
          </div>
          <button type="submit" disabled={loading} className="flex w-full items-center justify-center gap-2 rounded-xl bg-primary py-3 font-semibold text-primary-foreground hover:bg-primary/90 disabled:opacity-50">
            {loading ? <Loader2 className="h-5 w-5 animate-spin" /> : 'আপডেট করুন'}
          </button>
        </form>
      </div>
    </div>
...
function ProfileModal({ onClose }: { onClose: () => void }) {
  ...
  const handleChangePassword = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (user?.email) {
        ...
        if (signInError) {
          setPwError('বর্তমান পাসওয়ার্ড ভুল');
          setPwLoading(false);
          return;
        }
      }
      ...
      setPwError(err.message || 'পাসওয়ার্ড পরিবর্তন ব্যর্থ');
    } finally {
      setPwLoading(false);
    }
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative z-10 w-full max-w-md rounded-2xl bg-background p-6">
        <div className="mb-4 flex items-center justify-between">
          <h3 className="flex items-center gap-2 text-lg font-bold"><UserCog className="h-5 w-5" /> আমার প্রোফাইল</h3>
          <button onClick={onClose}><X className="h-5 w-5" /></button>
        </div>

        <div className="mb-4 flex gap-2 border-b border-border">
          <button onClick={() => setTab('profile')} className={`px-4 py-2 text-sm font-medium ${tab === 'profile' ? 'border-b-2 border-primary text-primary' : 'text-muted-foreground'}`}>প্রোফাইল</button>
          <button onClick={() => setTab('password')} className={`px-4 py-2 text-sm font-medium ${tab === 'password' ? 'border-b-2 border-primary text-primary' : 'text-muted-foreground'}`}>পাসওয়ার্ড পরিবর্তন</button>
        </div>
        ...
        {tab === 'profile' && (
          <form onSubmit={handleUpdateProfile} className="space-y-4">
            ...
            <div>
              <label className="mb-1 block text-sm font-medium">নাম</label>
              <input type="text" value={name} onChange={(e) => setName(e.target.value)} className="input-bangla" />
            </div>
            <div>
              <label className="mb-1 block text-sm font-medium">ফোন</label>
              <input type="tel" value={phone} onChange={(e) => setPhone(e.target.value)} className="input-bangla" placeholder="01XXXXXXXXX" />
            </div>
            <button type="submit" disabled={profileLoading} className="flex w-full items-center justify-center gap-2 rounded-xl bg-primary py-3 font-semibold text-primary-foreground hover:bg-primary/90 disabled:opacity-50">
              {profileLoading ? <Loader2 className="h-5 w-5 animate-spin" /> : 'প্রোফাইল আপডেট করুন'}
            </button>
          </form>
        )}
          ...
          <>
            {pwSuccess ? (
              <div className="rounded-xl border border-green-200 bg-green-50 p-4 text-center">
                ...
                <p className="font-medium text-green-800">Password successfully changed</p>
                <button onClick={onClose} className="mt-4 rounded-xl border border-border px-6 py-2 text-sm hover:bg-secondary">ঠিক আছে</button>
              </div>
            ) : (
              <form onSubmit={handleChangePassword} className="space-y-4">
                <div>
                  <label className="mb-1 block text-sm font-medium">বর্তমান পাসওয়ার্ড</label>
                  <input type="password" value={pwForm.currentPassword} onChange={(e) => setPwForm({ ...pwForm, currentPassword: e.target.value })} className="input-bangla" required />
                </div>
                <div>
                  <label className="mb-1 block text-sm font-medium">নতুন পাসওয়ার্ড</label>
                  <input type="password" value={pwForm.newPassword} onChange={(e) => setPwForm({ ...pwForm, newPassword: e.target.value })} className="input-bangla" placeholder="কমপক্ষি ৮ অক্ষর" required />
                </div>
                <div>
                  <label className="mb-1 block text-sm font-medium">নতুন পাসওয়ার্ড নিশ্চিত করুন</label>
                  <input type="password" value={pwForm.confirmPassword} onChange={(e) => setPwForm({ ...pwForm, confirmPassword: e.target.value })} className="input-bangla" required />
                </div>
                {pwError && <p className="rounded-lg bg-destructive/10 p-3 text-sm text-destructive">{pwError}</p>}
                <button type="submit" disabled={pwLoading} className="flex w-full items-center justify-center gap-2 rounded-xl bg-primary py-3 font-semibold text-primary-foreground hover:bg-primary/90 disabled:opacity-50">
                  {pwLoading ? <Loader2 className="h-5 w-5 animate-spin" /> : <><Lock className="h-5 w-5" /> পাসওয়ার্ড পরিবর্তন করুন</>}
                </button>
              </form>
            )}
          </>
  ...
  const loadLogs = async () => {
    ...
    try {
      const res = await fetch(FUNCTION_URL, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
        body: JSON.stringify({ action: 'list_audit_log' }),
      });
      if (res.ok) {
        const data = await res.json();
        setLogs(data.logs || []);
      ...
      toast('অডিট লগ লোড ব্যর্থ', 'error');
    }
    setLoading(false);
  };

  const actionLabels: Record<string, string> = {
    master_admin_created: 'মাস্টার অ্যাডমিন তৈরি',
    master_admin_enabled: 'মাস্টার অ্যাডমিন সক্রিয়',
    master_admin_disabled: 'মাস্টার অ্যাডমিন নিষ্ক্রিয়',
    master_admin_updated: 'মাস্টার অ্যাডমিন আপডেট',
    master_admin_password_reset: 'মাস্টার অ্যাডমিন পাসওয়ার্ড রিসেট',
    admin_created: 'অ্যাডমিন তৈরি',
    admin_enabled: 'অ্যাডমিন সক্রিয়',
    admin_disabled: 'অ্যাডমিন নিষ্ক্রিয়',
    admin_revoked: 'অ্যাডমিন অ্যাক্সেস বাতিল',
    password_reset_requested: 'পাসওয়ার্ড রিসেট অনুরোধ',
    role_changed: 'রোল পরিবর্তন',
    permission_changed: 'পারমিশন পরিবর্তন',
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative z-10 max-h-[80vh] w-full max-w-2xl overflow-y-auto rounded-2xl bg-background p-6">
        <div className="mb-4 flex items-center justify-between">
          <h3 className="flex items-center gap-2 text-lg font-bold"><History className="h-5 w-5" /> অডিট লগ</h3>
          <button onClick={onClose}><X className="h-5 w-5" /></button>
        </div>
        {loading ? (
          <div className="h-32 animate-pulse rounded-xl bg-secondary" />
        ) : logs.length === 0 ? (
          <p className="py-8 text-center text-muted-foreground">কোন অডিট লগ নেই</p>
        ) : (
            ...
            {logs.map((log) => (
              <div key={log.id} className="rounded-xl border border-border p-3 text-sm">
                ...
                <div className="mt-1 text-xs text-muted-foreground">
                  <span>অ্যাক্টর: {log.admin_email || '-'}</span>
                  {log.target_email && <span className="ml-3">টার্গেট: {log.target_email}</span>}
                </div>
              </div>
            ))}
          </div>
