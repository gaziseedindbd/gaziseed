...
type NavGroup = {
  label: string;
  items: { href: string; label: string; icon: any; masterOnly?: boolean }[];
};

const navGroups: NavGroup[] = [
  {
    label: '',
    items: [{ href: '/admin', label: 'ড্যাশবোর্ড', icon: LayoutDashboard }],
  },
  {
    label: 'অর্ডার',
    items: [
  ...
  {
    label: 'কাস্টমার',
    items: [{ href: '/admin/customers', label: 'কাস্টমার', icon: Users }],
  },
  {
    label: 'কন্টেন্ট',
    items: [
  ...
  {
    label: 'অ্যাডমিন',
    items: [{ href: '/admin/admin-management', label: 'এডমিন ম্যানেজমেন্ট', icon: UserCog, masterOnly: true }],
  },
];
    ...
    <div className="min-h-screen bg-secondary/10 text-foreground">
      ...
      <div className="sticky top-0 z-45 flex items-center justify-between border-b border-border bg-background px-4 py-3 lg:hidden">
        ...
        <div className="flex items-center gap-2">
          <button onClick={toggleTheme} className="rounded-lg border border-border p-2 hover:bg-secondary">
        ...
        <div className="flex h-full flex-col">
          <div className="flex items-center justify-between border-b border-border p-4">
            <Link href="/admin" className="flex items-center gap-2">
              <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-primary text-primary-foreground"><span className="text-sm font-bold">G</span></div>
              <span className="font-bold text-primary">SEED BARI</span>
            </Link>
            <div className="flex items-center gap-1">
              <button onClick={toggleTheme} className="hidden lg:flex rounded-lg border border-border p-2 hover:bg-secondary" title="থিম পরিবর্তন করুন">
                {theme === 'dark' ? <Sun className="h-4 w-4 text-amber-400" /> : <Moon className="h-4 w-4" />}
              </button>
          ...
          <div className="border-b border-border px-4 py-3">
            <div className="flex items-center gap-2">
              ...
              <div>
                <p className="text-xs text-muted-foreground truncate max-w-[140px]">{adminEmail}</p>
                <span className={`flex items-center gap-1 text-xs font-semibold ${isMasterAdmin ? 'text-primary' : 'text-muted-foreground'}`}>
          ...
          <nav className="flex-1 overflow-y-auto p-3">
            {navGroups.map((group, gidx) => {
              const items = group.items.filter((item) => !item.masterOnly || isMasterAdmin);
              if (items.length === 0) return null;
              return (
                <div key={gidx} className="mb-4">
                  {group.label && <p className="mb-1 px-3 text-xs font-semibold uppercase tracking-wider text-muted-foreground/60">{group.label}</p>}
                  {items.map((item) => (
                    <Link key={item.href} href={item.href} onClick={() => setSidebarOpen(false)}
                      className={`mb-0.5 flex items-center gap-3 rounded-lg px-3 py-2 text-sm font-medium transition-colors ${isActive(item.href) ? 'bg-primary text-primary-foreground' : 'text-foreground/70 hover:bg-secondary'}`}>
