...
export function SiteHeader() {
  ...
  return (
    <header className="fixed inset-x-0 top-0 z-50 bg-white/98 shadow-[0_12px_40px_-24px_rgba(5,46,22,.48)] backdrop-blur-xl">
      ...
      {mobileMenuOpen && <div className="border-t border-slate-100 bg-white shadow-2xl md:hidden"><div className="p-3"><div className="mb-2 flex items-center gap-2 px-1 text-[11px] font-extrabold uppercase tracking-[0.16em] text-slate-400">Menu<span className="h-px flex-1 bg-slate-100" /></div><button type="button" onClick={() => { setMobileMenuOpen(false); setSearchOpen(true); }} className="mb-3 flex w-full items-center gap-3 rounded-2xl border border-emerald-100 bg-emerald-50/70 p-3 text-left text-sm font-bold text-emerald-900"><Search className="h-5 w-5 text-emerald-700" />{t('পণ্য ও বীজ খুঁজুন...', 'Search products & seeds...')}</button><Link href="/account" onClick={() => setMobileMenuOpen(false)} className="mb-3 flex w-full items-center gap-3 rounded-2xl border border-emerald-200 bg-emerald-50 p-3 text-left text-sm font-black text-emerald-900"><User className="h-5 w-5 text-emerald-700" />{t('অ্যাকাউন্ট / লগইন', 'Account / Login')}</Link>{!isAdminRoute && <div className="mb-3"><CountrySelector mobile /></div>}<div className="mb-3 rounded-2xl border border-emerald-100 bg-gradient-to-br from-emerald-50/90 to-white p-3 shadow-sm"><div className="mb-2 flex items-center justify-between"><span className="text-[10px] font-black uppercase tracking-[0.14em] text-emerald-900">Language</span><span className="text-[9px] font-bold text-slate-400">{country === 'IN' ? 'বাংলা · English · हिन्दी' : 'বাংলা · English'}</span></div><div className="flex items-center gap-1.5 rounded-xl border border-emerald-100 bg-white p-1">{(country === 'IN' ? [['bn', 'বাংলা'], ['en', 'EN'], ['hi', 'हिन्दী']] : [['bn', 'বাংলা'], ['en', 'EN']]).map(([code, label]) => (<button key={code} type="button" onClick={() => setLang(code)} className={lang === code ? 'flex-1 rounded-lg bg-emerald-800 px-3 py-2 text-xs font-black text-white shadow-sm' : 'flex-1 rounded-lg px-3 py-2 text-xs font-black text-emerald-900 transition hover:bg-emerald-50'} aria-pressed={lang === code}>{label}</button>))}</div></div><div className="grid grid-cols-2 gap-2">{navLinks.map((link) => { const Icon = link.icon; const active = isActive(link.href); return (<Link key={link.href} href={link.href} className={`flex items-center gap-2 rounded-2xl border p-3 text-sm font-bold transition ${active ? 'border-emerald-200 bg-emerald-50 text-emerald-800' : 'border-slate-100 bg-slate-50 text-slate-700 hover:border-emerald-100 hover:bg-emerald-50/50'}`}><Icon className="h-5 w-5 text-emerald-700" />{link.label}</Link>); })}</div></div></div>}
    </header>
  );
}
