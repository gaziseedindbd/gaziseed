...
export function SiteHeader() {
  ...
  return (
    <header className="fixed inset-x-0 top-0 z-50 bg-white/98 shadow-[0_12px_40px_-24px_rgba(5,46,22,.48)] backdrop-blur-xl">
      ...
      <div className="mx-auto flex max-w-[1600px] items-center gap-4 px-4 py-2.5 sm:px-6 lg:gap-5 lg:px-8 lg:py-3">
        ...
        <div className="ml-auto hidden shrink-0 items-center gap-1.5 lg:flex">
          <div className={`relative flex items-center overflow-hidden rounded-2xl border transition-all duration-200 ${searchOpen ? 'border-emerald-400 bg-white ring-4 ring-emerald-600/10 shadow-sm' : 'border-emerald-100 bg-emerald-50/65 hover:border-emerald-200 hover:bg-white'}`}>
            <button type="button" onClick={() => setSearchOpen(true)} aria-label={t('খুঁজুন', 'Search')} className="flex shrink-0 items-center justify-center pl-3 text-emerald-700"><Search className="h-5 w-5" /></button>
            <input ref={searchInputRef} value={searchQuery} onFocus={() => setSearchOpen(true)} onChange={(e) => { setSearchQuery(e.target.value); setSearchOpen(true); }} onKeyDown={(e) => { if (e.key === 'Enter') submitSearch(); if (e.key === 'Escape') setSearchOpen(false); }} type="search" placeholder={t('পণ্য খুঁজুন...', 'Search products...')} className="w-[150px] bg-transparent px-2.5 py-2.5 text-[12px] font-semibold text-slate-800 outline-none placeholder:text-slate-400 2xl:w-[180px]" aria-label={t('পণ্য খুঁজুন', 'Search products')} />
      ...
      {mobileMenuOpen && <div className="border-t border-slate-100 bg-white shadow-2xl md:hidden"><div className="p-3"><div className="mb-2 flex items-center gap-2 px-1 text-[11px] font-extrabold uppercase tracking-[0.16em] text-slate-400">Menu<span className="h-px flex-1 bg-slate-100" /></div><button type="button" onClick={() => { setMobileMenuOpen(false); setSearchOpen(true); }} className="mb-3 flex w-full items-center gap-3 rounded-2xl border border-emerald-100 bg-emerald-50/70 p-3 text-left text-sm font-bold text-emerald-900"><Search className="h-5 w-5 text-emerald-700" />{t('পণ্য ও বীজ খুঁজুন...', 'Search products & seeds...')}</button><div className="mb-3"><CountrySelector mobile /></div><div className="grid grid-cols-2 gap-2">{navLinks.map((link) => { const Icon = link.icon; const active = isActive(link.href); return (<Link key={link.href} href={link.href} className={`flex items-center gap-2 rounded-2xl border p-3 text-sm font-bold transition ${active ? 'border-emerald-200 bg-emerald-50 text-emerald-800' : 'border-slate-100 bg-slate-50 text-slate-700 hover:border-emerald-100 hover:bg-emerald-50/50'}`}><Icon className="h-5 w-5 text-emerald-700" />{link.label}</Link>); })}</div></div></div>}
    </header>
  );
}
