    ...
    <article className="group relative flex h-full min-w-0 flex-col overflow-hidden rounded-[1.25rem] border border-slate-200/80 bg-white shadow-[0_8px_30px_-22px_rgba(15,23,42,.55)] ring-1 ring-transparent transition-all duration-300 ease-out hover:-translate-y-1.5 hover:border-emerald-300/80 hover:shadow-[0_24px_60px_-30px_rgba(5,150,105,.45)] hover:ring-emerald-500/10 sm:rounded-[1.5rem]">
      <Link href={`/product/${product.slug}`} className="group block min-w-0">
        <div className="relative aspect-[1/1.02] w-full overflow-hidden bg-gradient-to-br from-emerald-50 via-lime-50 to-stone-100 sm:aspect-[.96]">
          ...
          {discount > 0 && <span className="absolute bottom-2.5 left-2.5 inline-flex items-center gap-1 rounded-lg bg-red-500 px-2 py-1 text-[9px] font-black text-white shadow-lg sm:bottom-3 sm:left-3 sm:rounded-xl sm:px-2.5 sm:py-1.5 sm:text-[10px]">{discount}% {t('ছাড়', 'OFF')}</span>}
          {product.packet_weight && <span className="absolute bottom-2.5 right-2.5 inline-flex items-center gap-1 rounded-lg border border-white/80 bg-white/90 px-2 py-1 text-[8px] font-extrabold text-slate-700 shadow-md backdrop-blur-md sm:bottom-3 sm:right-3 sm:px-2.5 sm:py-1.5 sm:text-[9px]"><Package className="h-2.5 w-2.5 text-emerald-700 sm:h-3 sm:w-3" />{product.packet_weight}</span>}
