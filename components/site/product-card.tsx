...
export function ProductCard({ product, stackedActions = false }: { product: Product; stackedActions?: boolean }) {
  ...
  return (
    <article className="group relative flex h-full min-w-0 flex-col overflow-hidden rounded-[1.45rem] border border-slate-200/70 bg-white shadow-[0_12px_34px_-24px_rgba(15,23,42,.48)] ring-1 ring-black/[0.02] transition-all duration-300 ease-out hover:-translate-y-1.5 hover:border-emerald-300/70 hover:shadow-[0_28px_70px_-34px_rgba(5,150,105,.34)] hover:ring-emerald-500/10 sm:rounded-[1.7rem]">
      <Link href={`/product/${product.slug}`} className="group block min-w-0">
        <div className="relative mx-2 mt-2 aspect-[1/1.03] overflow-hidden rounded-[1.15rem] bg-gradient-to-br from-[#f7fbf8] via-[#eef7f1] to-[#f6f2e8] ring-1 ring-emerald-900/[0.04] sm:mx-2.5 sm:mt-2.5 sm:rounded-[1.3rem]">
          ...
          {product.image ? (
            <img
              ...
              className="relative z-[1] h-full w-full object-contain p-4 transition-transform duration-500 ease-out group-hover:scale-[1.045] sm:p-5"
              loading="lazy"
            />
          ) : (
          ...
          <div className="absolute inset-x-3 top-3 z-[3] flex items-start justify-between sm:inset-x-3.5 sm:top-3.5">
            {badge ? (
              <span className="rounded-full border border-white/70 bg-emerald-700/95 px-3 py-1.5 text-[9px] font-black text-white shadow-lg shadow-emerald-900/15 backdrop-blur sm:px-3.5 sm:text-[10px]">
                {badge}
              </span>
            ) : (
              <span />
          ...
          {discount > 0 && (
            <span className="absolute bottom-3 left-3 z-[3] inline-flex items-center rounded-full border border-white/20 bg-red-500 px-2.5 py-1.5 text-[9px] font-black text-white shadow-lg shadow-red-900/10 sm:left-3.5 sm:px-3 sm:text-[10px]">
              {discount}% {t('ছাড়', 'OFF', 'छूट')}
            </span>
          )}

          {product.packet_weight && (
            <span className="absolute bottom-3 right-3 z-[3] inline-flex items-center gap-1.5 rounded-full border border-white/75 bg-white/92 px-2.5 py-1.5 text-[9px] font-extrabold text-slate-700 shadow-lg shadow-slate-900/10 backdrop-blur-md sm:right-3.5 sm:px-3 sm:text-[10px]">
        ...
        <div className="flex flex-col gap-3 px-3.5 pb-3.5 pt-4 sm:gap-3.5 sm:px-4.5 sm:pb-4 sm:pt-4.5 lg:px-5 lg:pb-4.5 lg:pt-5">
          <div className="min-w-0">
            ...
            <h3 className="line-clamp-2 text-[13px] font-black leading-[1.35] tracking-[-.012em] text-slate-900 sm:text-[15px] lg:text-[16px]">
              {name}
            </h3>
            ...
            {product.short_description && (
              <p className="mt-1.5 line-clamp-2 text-[9px] leading-[1.55] text-slate-500 sm:mt-2 sm:text-[10px]">
                {product.short_description}
              </p>
            )}
          </div>
      ...
      <div className="mt-auto px-3.5 pb-3.5 sm:px-4 sm:pb-4 lg:px-5 lg:pb-5">
        <div className="mb-3 flex min-w-0 items-end justify-between gap-2 border-t border-slate-100 pt-3 sm:pt-3.5">
          <div className="flex min-w-0 flex-wrap items-baseline gap-x-2 gap-y-0.5">
            <span className="text-[1.15rem] font-black tracking-tight text-emerald-800 sm:text-[1.4rem]">
              {formatPrice(price)}
            </span>
            {discount > 0 && (
        ...
        <div className="mt-2.5 flex items-center justify-center gap-3 text-[8px] font-semibold text-slate-400 sm:text-[9px]">
