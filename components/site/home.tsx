...
export default function Home() {
  ...
  return (
    <div className={`${themeClass} home-premium-scope`}>
      ...
      {isSectionEnabled('hero_slider') && <section className="section-pad home-hero-section"><div className="container-custom"><div className="hero-wrap relative overflow-hidden"><div className="hero-inner relative block p-0 aspect-[2/1] sm:aspect-[16/5]">{banners.length > 0 ? banners.map((banner, idx) => <div key={banner.id} className={`absolute inset-0 transition-opacity duration-700 ${idx === currentBanner ? 'block' : 'hidden'}`}><div className="hero-inner relative block h-full p-0">{banner.desktop_image && <div className="hero-image-wrap block h-full w-full"><picture className="block h-full w-full"><source media="(max-width: 767px)" srcSet={banner.mobile_image || banner.desktop_image} /><img src={banner.desktop_image} alt={banner.title || 'GAZI SEED'} className="block h-full w-full rounded-2xl object-cover shadow-lg" loading={idx === 0 ? 'eager' : 'lazy'} /></picture></div>}{(banner.title || banner.subtitle || banner.cta_text) && <div className="hero-content-overlay absolute inset-x-0 bottom-0 z-10 p-4 sm:p-6 lg:p-8"><div className="max-w-xl rounded-2xl bg-black/25 p-4 text-white backdrop-blur-[2px] sm:p-5">{banner.title && <h2 className="hero-title">{tDb(banner.title)}</h2>}{banner.subtitle && <p className="hero-subtitle">{tDb(banner.subtitle)}</p>}{banner.cta_text && <Link href={banner.cta_url || '/all-products'} className="hero-btn inline-flex">{tDb(banner.cta_text)}<ChevronRight className="h-4 w-4" /></Link>}</div></div>}</div></div>) : <div className="h-full w-full rounded-2xl bg-secondary/20" aria-hidden="true" />}{banners.length > 1 && <div className="absolute bottom-4 left-1/2 z-20 flex -translate-x-1/2 gap-2">{banners.map((_, idx) => <button key={idx} onClick={() => setCurrentBanner(idx)} className={`h-2 rounded-full transition-all ${idx === currentBanner ? 'w-8 bg-primary' : 'w-2 bg-primary/30'}`} aria-label={`Banner ${idx + 1}`}></button>)}</div>}</div></div></div></section>}
      {isSectionEnabled('featured_categories') && <section className="section-pad home-category-section"><div className="container-custom"><div className="mb-6 flex items-center justify-between"><div><h2 className="section-heading">{t('জনপ্রিয় ক্যাটাগরি', 'Popular Categories')}</h2><p className="section-subheading">{t('আপনার পছন্দের বীজ এক নজরে', 'Explore seeds by category')}</p></div><Link href="/categories" className="view-all-link">{t('সব ক্যাটাগরি দেখুন →', 'View All Categories →')}</Link></div><div className="cat-grid">{categoriesLoaded ? categories.slice(0, 8).map((cat, idx) => { const categoryHref = cat.slug === 'combo-packages' ? '/combos' : `/category/${cat.slug}`; return <Link key={cat.id} href={categoryHref} className={`cat-card group${idx >= 4 ? ' !hidden sm:!flex' : ''}`}><div className="cat-icon-wrap">{cat.image ? <img src={cat.image} alt={lang === 'en' ? (cat.name_en || cat.name_bn) : cat.name_bn} className="h-full w-full rounded-full object-cover" loading="lazy" /> : <Sprout className="h-8 w-8 text-primary" />}</div><span className="cat-label">{lang === 'en' ? (cat.name_en || cat.name_bn) : cat.name_bn}</span></Link>; }) : Array.from({ length: 8 }, (_, idx) => <div key={`category-skeleton-${idx}`} className={`cat-card category-card-skeleton${idx >= 4 ? ' !hidden sm:!flex' : ''}`} aria-hidden="true"><div className="cat-icon-wrap"><div className="category-skeleton-image" /></div><span className="cat-label"><span className="category-skeleton-label" /></span></div>)}</div></div></section>}
