...
export default function Home() {
  ...
  useEffect(() => {
    ...
    Promise.allSettled([
      getBanners(),
      getCategories(),
      getProducts({ is_featured: true, limit: 10 }),
      getProducts({ is_best_seller: true, limit: 10 }),
      getProducts({ is_new_arrival: true, limit: 10 }),
      getProducts({ is_seasonal: true, limit: 10 }),
      getServices(),
      getTestimonials(),
      getBlogPosts(),
      getSiteSettings(),
      ...
      bRes, cRes, fpRes, bsRes, naRes, ssRes, svRes, tRes, bpRes, stRes, hsRes, tmsRes
    ]) => {
      ...
      setSeasonal(ss);
      setServices(sv);
      setTestimonials(tVal);
      setBlogPosts(bp);
      setSettings(st);
      setSections(hs);
      setThisMonthSeeds(tms);
  ...
  const isSectionEnabled = (key: string) =>
    sections.length === 0 || sections.find((s) => s.section_key === key)?.is_enabled !== false;
  ...
  return (
    <div className={`${themeClass} home-premium-scope`}>
      ...
      {isSectionEnabled('hero_slider') && banners.length > 0 && (
        <section className="section-pad home-hero-section">
          <div className="container-custom">
            <div className="hero-wrap relative overflow-hidden">
              {banners.map((banner, idx) => (
                <div key={banner.id} className={`transition-opacity duration-700 ${idx === currentBanner ? 'block' : 'hidden'}`}>
                  <div className="hero-inner grid items-center gap-4 px-6 sm:px-10 lg:grid-cols-2 lg:px-16">
                    <div>
                      {banner.title && <h2 className="hero-title">{tDb(banner.title)}</h2>}
                      {banner.subtitle && <p className="hero-subtitle">{tDb(banner.subtitle)}</p>}
                      {banner.cta_text && <Link href={banner.cta_url || '/all-products'} className="hero-btn">{tDb(banner.cta_text)}<ChevronRight className="h-4 w-4" /></Link>}
                    </div>
                    {banner.desktop_image && (
                      <div className="block"><picture><source media="(max-width: 767px)" srcSet={banner.mobile_image || banner.desktop_image} /><img src={banner.desktop_image} alt={banner.title} className="h-56 w-full rounded-2xl object-cover shadow-lg sm:h-72" loading="eager" /></picture></div>
                    )}
                  </div>
                </div>
              ))}
      ...
      {isSectionEnabled('featured_categories') && categories.length > 0 && (
        <section className="section-pad home-category-section">
          <div className="container-custom">
            <div className="mb-6 flex items-center justify-between">
              <div><h2 className="section-heading">{t('জনপ্রিয় ক্যাটাগরি', 'Popular Categories')}</h2><p className="section-subheading">{t('আপনার পছন্দের বীজ এক নজরে', 'Explore seeds by category')}</p></div>
              <Link href="/categories" className="view-all-link">{t('সব ক্যাটাগরি দেখুন →', 'View All Categories →')}</Link>
            </div>
            <div className="cat-grid">
              {categories.slice(0, 8).map((cat, idx) => {
                const categoryHref = cat.slug === 'combo-packages' ? '/combos' : `/category/${cat.slug}`;
                return <Link key={cat.id} href={categoryHref} className={`cat-card group${idx >= 4 ? ' !hidden sm:!flex' : ''}`}>
                  <div className="cat-icon-wrap">{cat.image ? <img src={cat.image} alt={lang === 'en' ? (cat.name_en || cat.name_bn) : cat.name_bn} className="h-full w-full rounded-full object-cover" loading="lazy" /> : <Sprout className="h-8 w-8 text-primary" />}</div>
                  <span className="cat-label">{lang === 'en' ? (cat.name_en || cat.name_bn) : cat.name_bn}</span>
                </Link>;
              })}
            </div>
          </div>
      ...
      <section className="home-trust-strip">
        <div className="container-custom home-trust-grid">
          <TrustItem icon={Sprout} title={t('১০০% অরিজিনাল বীজ', '100% Original Seeds')} text={t('বিশ্বস্ত উৎস থেকে', 'From trusted sources')} />
          <TrustItem icon={Truck} title={t('সারা দেশে ডেলিভারি', 'Nationwide Delivery')} text={t('দ্রুত ও নিরাপদ', 'Fast & secure')} />
          <TrustItem icon={ShieldCheck} title={t('নিরাপদ পেমেন্ট', 'Secure Payment')} text={t('বিকাশ, নগদ, কার্ড ও COD', 'bKash, Nagad, card & COD')} />
          <TrustItem icon={Headphones} title={t('কাস্টমার সাপোর্ট', 'Customer Support')} text={t('সবসময় আপনার পাশে', 'Always here for you')} />
        </div>
      </section>
      ...
      {(promoImage1 || promoImage2) && (
        <section className="section-pad home-promo-section">
          <div className="container-custom home-promo-grid">
            <PromoCard image={promoImage1} title={t('চাষাবাদ গাইড', 'Growing Guide')} text={t('সঠিক বীজ, সঠিক পদ্ধতি — বেশি ফলন', 'Right seed, right method — better yield')} href="/blog" button={t('বিস্তারিত দেখুন', 'Explore Guide')} />
            <PromoCard image={promoImage2} title={t('কৃষকের গল্প', 'Farmer Stories')} text={t('আমাদের বীজে সাফল্যের অনুপ্রেরণামূলক গল্প', 'Inspiring success stories from farmers')} href="/blog" button={t('দেখুন এখন', 'View Stories')} />
          </div>
        </section>
      )}

      <section className="home-stats-strip">
        <div className="container-custom home-stats-grid">
          <StatItem icon={Users} title={t('কৃষকের আস্থা', 'Farmer Trust')} text={t('মানসম্মত বীজ', 'Quality seeds')} />
          <StatItem icon={Sprout} title={t('বীজের সমৃদ্ধ সংগ্রহ', 'Rich Seed Collection')} text={t('বিভিন্ন জাতের বীজ', 'Many varieties')} />
          <StatItem icon={MapPin} title={t('সারাদেশে সেবা', 'Nationwide Service')} text={t('ডেলিভারি সুবিধা', 'Delivery available')} />
          <StatItem icon={ShieldCheck} title={t('নিরাপদ কেনাকাটা', 'Safe Shopping')} text={t('বিশ্বস্ত অর্ডার প্রসেস', 'Trusted order process')} />
        </div>
      </section>
      ...
      {isSectionEnabled('why_choose_us') && (
        <section className="section-pad feature-section-bg legacy-feature-section">
          <div className="container-custom"><h2 className="mb-8 text-center section-heading">{t('কেন বেছে নেবেন সিড বাড়ি?', 'Why Choose SEED BARI?')}</h2><div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">{[
            { icon: Sprout, title: t('অরিজিনাল বীজ', 'Original Seeds'), desc: t('১০০% অরিজিনাল ও অঙ্কুরিত বীজ', '100% original & germinated seeds') },
            { icon: Truck, title: t('সারাদেশে ডেলিভারি', 'Nationwide Delivery'), desc: t('ক্যাশ অন ডেলিভারি সারাদেশে', 'Cash on delivery nationwide') },
            { icon: ShieldCheck, title: t('মানি ব্যাক গ্যারান্টি', 'Money Back Guarantee'), desc: t('পণ্য সন্তুষ্টিজনক না হলে ফেরত', 'Return if not satisfied') },
            { icon: Phone, title: t('২৪/৭ সাপোর্ট', '24/7 Support'), desc: t('ফোন ও হোয়াটসঅ্যাপে সাপোর্ট', 'Phone & WhatsApp support') },
          ].map((item, idx) => <div key={idx} className="why-card"><div className="why-icon-wrap"><item.icon className="h-7 w-7" /></div><h3 className="why-title">{item.title}</h3><p className="why-desc">{item.desc}</p></div>)}</div></div>
        </section>
      )}
...
function TrustItem({ icon: Icon, title, text }: { icon: React.ComponentType<{ className?: string }>; title: string; text: string }) {
  return <div className="home-trust-item"><div className="home-trust-icon"><Icon className="h-6 w-6" /></div><div><h3>{title}</h3><p>{text}</p></div></div>;
}

function StatItem({ icon: Icon, title, text }: { icon: React.ComponentType<{ className?: string }>; title: string; text: string }) {
  return <div className="home-stat-item"><div className="home-stat-icon"><Icon className="h-6 w-6" /></div><div><strong>{title}</strong><span>{text}</span></div></div>;
}

function PromoCard({ image, title, text, href, button }: { image: string; title: string; text: string; href: string; button: string }) {
  return <Link href={href} className="home-promo-card group">{image && <img src={image} alt="" loading="lazy" />}<div className="home-promo-overlay" /><div className="home-promo-content"><h3>{title}</h3><p>{text}</p><span>{button}<ChevronRight className="h-4 w-4" /></span></div></Link>;
}

function ProductSection({ title, subtitle, products, theme }: { title: string; subtitle?: string; products: Product[]; theme: HomePageTheme }) {
  const { t } = useLang();
  if (products.length === 0) return null;
  return <section className="section-pad home-product-section"><div className="container-custom"><div className="mb-6 flex items-center justify-between"><div><h2 className="section-heading">{title}</h2>{subtitle && <p className="section-subheading">{subtitle}</p>}</div><Link href="/all-products" className="view-all-link">{t('সব দেখুন →', 'View All →')}</Link></div><div className="product-grid">{products.map((product) => <ProductCard key={product.id} product={product} stackedActions />)}</div></div></section>;
}
