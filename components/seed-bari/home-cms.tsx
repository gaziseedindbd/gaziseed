      ...
      {campaigns.length > 0 && (
        <section className="mb-10 rounded-3xl border bg-white p-6 md:p-8">
          <div className="mb-5">
            ...
            <h2 className="mt-1 text-2xl font-black md:text-3xl">Current SEED BARI offers</h2>
          </div>
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {campaigns.slice(0, 6).map((campaign) => (
              ...
              >
                ...
                {campaign.offer_text && <p className="mt-2 text-sm text-gray-600">{campaign.offer_text}</p>}
              </Link>
            ))}
          </div>
        </section>
