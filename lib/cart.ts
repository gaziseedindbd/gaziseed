...
function getStoreCountry():StoreCountry{
  ...
  const value=document.cookie.split(';').map((part)=>part.trim()).find((part)=>part.startsWith(`${COUNTRY_KEY}=`))?.split('=')[1];
...
export async function loadCart(){
  ...
  if(user){
    const {data:rows}=await s.from('cart_items').select('id,product_id,variant_id,quantity').eq('user_id',user.id).order('created_at');
    if(!rows?.length)return [];
    const productIds=[...new Set(rows.map((x:any)=>x.product_id))];
    const variantIds=[...new Set(rows.map((x:any)=>x.variant_id).filter(Boolean))] as string[];
    const [{data:products},{data:variants}]=await Promise.all([
      s.from('products').select('id,name_en,name_bn,regular_price,sale_price,stock,sku,active,country').in('id',productIds).eq('active',true).eq('country',country),
    ...
    return rows.filter((i:any)=>productMap.has(i.product_id)&&(!i.variant_id||variantMap.has(i.variant_id))).map((i:any)=>({...i,product:productMap.get(i.product_id),variant:i.variant_id?variantMap.get(i.variant_id):undefined}));
  }
