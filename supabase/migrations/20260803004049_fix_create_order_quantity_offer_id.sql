-- Update create_order to insert quantity_offer_id into the new column instead of bundle_id
...
BEGIN
  ...
  LOOP
    ...
    IF (v_item->>'quantity_offer_id') IS NOT NULL THEN
      ...
      IF NOT FOUND THEN
        RETURN jsonb_build_object('error', 'Invalid quantity offer');
      END IF;
    ...
    ELSIF (v_item->>'bundle_id') IS NOT NULL THEN
      ...
      IF NOT FOUND THEN
        RETURN jsonb_build_object('error', 'Invalid bundle');
      END IF;
    ...
    ELSE
      ...
      IF NOT FOUND THEN
        RETURN jsonb_build_object('error', 'Invalid product');
      END IF;
