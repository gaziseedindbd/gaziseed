/*
# Fix create_order: deduct bundle quantity from stock

The existing create_order function deducts 1 unit per bundle order.
It should deduct the bundle's quantity from stock.

Example: 3-packet bundle = deduct 3 from stock, not 1.

Also fixes: use landing_slug as utm_campaign fallback for attribution.
*/
...
BEGIN
  ...
  LOOP
    ...
    IF (v_item->>'bundle_id') IS NOT NULL THEN
      ...
      -- Deduct bundle quantity from stock (not order quantity)
